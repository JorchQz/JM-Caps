...
**No hay cupones y no debe haberlos mientras el cobro sea en persona.** Se construyeron y se quitaron el 21 de septiembre de 2026 (migración `quitar_cupones`): un código que el cliente no puede canjear solo, porque no hay pago en línea, no es un cupón — es un descuento que el vendedor teclea de su lado, y para eso ya están las ofertas. Si algún día hay pago en línea, la migración `precios_ofertas_y_cupones` tiene el diseño completo para recuperarlo.

`precio_efectivo` en la base es la única definición del precio que se cobra, y la usan la vista pública y la venta. Hay una copia en TypeScript (`precioEfectivo`, en `packages/db/src/descuentos.ts`) para que el panel pinte el resultado sin ir a la red mientras se captura. Si una cambia, la otra también.
...
- `id` (uuid, pk), `fecha`, `total_mxn` (suma de las piezas con su oferta ya aplicada), `metodo_pago` (enum: efectivo/spei/tarjeta/otro), `canal` (enum: local_colotlan/local_tepatitlan/envio_nacional), `cliente_nombre`, `cliente_telefono`
...
- **`registrar_venta(p_unidad_ids, p_metodo_pago, p_canal, p_cliente_nombre, p_cliente_telefono, p_notas)`** — venta completa (encabezado, items al precio con la oferta ya aplicada, y cambio de estado). Bloquea las filas antes de cobrar para que dos ventas simultáneas no vendan la misma pieza.
